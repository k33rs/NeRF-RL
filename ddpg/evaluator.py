        ...
        for episode in range(1, self.episodes + 1):
            ...
            for step in range(1, self.episode_steps + 1):
                ...
                if self.tensorboard:
                    ...
                    self.writer.add_scalar(f'[eval]_b{batch_idx}_ep{episode}/done_count', info['done_count'], step)
                    self.writer.add_scalar(f'[eval]_b{batch_idx}_ep{episode}/done_ratio', info['done_ratio'], step)
                # update
            ...
            if self.with_img:
                fig, ax = show_img(info['img'], fig, ax, title=log, write_path=self.save_path)
            if self.tensorboard:
                ...
                self.writer.add_scalar(f'[eval]_b{batch_idx}/ep_done_count', info['done_count'], episode)
                self.writer.add_scalar(f'[eval]_b{batch_idx}/ep_done_ratio', info['done_ratio'], episode)
                self.writer.flush()
