...
def clip_to_angle(
    ...
    uuT = u @ u.transpose(1, 2) # outer product
    # rotate vectors that need it
    num_vec = axis.size(0)
    num_angles = clip_angle_rad.size(0)
    clip_angle_rad = clip_angle_rad.repeat(num_vec // num_angles)
    cos_theta = torch.cos(clip_angle_rad).view(-1, 1, 1)
    sin_theta = torch.sin(clip_angle_rad).view(-1, 1, 1)
    I = torch.stack([torch.eye(3)] * num_vec).to(dir.device)
    ...
    clipped = dir * ~mask + rotated * mask
    # normalize (happens only if direction is antipodal w.r.t. hemi_axis)
    norm, one = clipped.norm(dim=-1), torch.ones(1, device=dir.device)
    if not torch.isclose(norm, one).all():
        clipped /= clipped.norm(dim=-1, keepdim=True)
    return clipped
...
def clip_to_image(
    ...
    img_idx = torch.arange(dir.size(0), device=dir.device) // imsize
